...
class _ScheduleScreenState extends State<ScheduleScreen> with TickerProviderStateMixin {
  ...
  final List<String> _allDays = ['LUN', 'MAR', 'MER', 'GIO', 'VEN', 'SAB', 'DOM'];
  ...
  Future<void> _loadSchedule() async {
    try {
      ...
      _scheduleByDay.forEach((key, list) {
        list.sort((a, b) => (a['startTime'] as String).compareTo(b['startTime'] as String));
      });
      ...
      if (_activeDayIndexes.isNotEmpty) {
        ...
        _tabController = TabController(length: _activeDays.length, vsync: this, initialIndex: initialIndex);
      }
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Errore durante il caricamento del palinsesto. Riprova più tardi.';
        });
      }
    }
  }
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(16), // Ridotto raggio curvatura
                  boxShadow: [
                    BoxShadow(
                ...
                child: ClipRRect(
                  ...
                  child: Image.asset(
                    ...
                    errorBuilder: (_, _, _) => const Center(
                      child: Text('Lady\nRadio', textAlign: TextAlign.center, style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.bold, fontSize: 24)),
                    ),
                  ),
                ),
              ),
              ...
              const Padding(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: Color(0xFF868B9E), fontSize: 12.5), // Ridotto da 13.5
                ),
              ),
              ...
              if (_isLoading)
                 ...
                 const Expanded(child: Center(child: Text('Nessun programma per il palinsesto', style: TextStyle(color: Colors.grey))))
              else ...[
                ...
                Container(
                  ...
                  child: TabBar(
                    ...
                    indicator: const UnderlineTabIndicator(
                      borderSide: BorderSide(color: Color(0xFF801A7B), width: 2), // Mockup has a slightly darker purple underline
                    ),
                    indicatorSize: TabBarIndicatorSize.label,
                    labelColor: const Color(0xFF14003E), // dark text
                    labelStyle: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14),
                    unselectedLabelColor: const Color(0xFF14003E).withValues(alpha: 0.8),
                    unselectedLabelStyle: const TextStyle(fontWeight: FontWeight.w500, fontSize: 14),
                    tabs: _activeDays.map((day) => Tab(text: day)).toList(),
                  ),
                ),
                            ...
                            return GestureDetector(
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) => ProgramScreen(programData: item),
                                  ),
                                );
                              },
                              child: Padding(
